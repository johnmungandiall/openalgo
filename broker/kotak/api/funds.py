            ...
            if positions_response.get("stat", "").lower() == "ok" and data is not None:
                ...
                for position in positions:
                    ...
                    if net_qty == 0:
                        ...
                        # Open position - use broker-provided rpnl directly (like Flattrade)
                        # This includes all charges and is more accurate than manual calculation
                        rpnl = safe_float(position.get("rpnl"))
                        total_realised += rpnl

                        logger.debug(
                            f"Open Position {position.get('trdSym')}: "
                            f"net_qty={net_qty}, rpnl={rpnl:.2f}"
                        )
